...
async fn device_create_read_update_delete() {
    ...
    // READ
    let row = sqlx::query("SELECT id, name, category, protocol_type, state, workspace_id FROM things WHERE id = ?1")
        .bind(&device_id)
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    assert_eq!(row.get::<String, _>("category"), "sensor");
    ...
    // UPDATE
    sqlx::query("UPDATE things SET name = ?1, updated_at = ?2 WHERE id = ?3")
        .bind("Updated Sensor")
    ...
    let updated = sqlx::query_scalar::<_, String>("SELECT name FROM things WHERE id = ?1")
        .bind(&device_id)
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    // DELETE
    sqlx::query("DELETE FROM things WHERE id = ?1")
        .bind(&device_id)
        .execute(&pool)
        .await
        .unwrap();

    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM things WHERE id = ?1")
        .bind(&device_id)
        .fetch_one(&pool)
        .await
        .unwrap();
