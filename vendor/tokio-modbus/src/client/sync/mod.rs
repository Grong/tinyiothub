...
impl Reader for Context {
    fn read_coils(&mut self, addr: Address, cnt: Quantity) -> Result<Vec<Coil>> {
        self.core.block_on(async {
            tokio::select! {
                    ctx = self.async_ctx.read_coils(addr, cnt) => {
                        ctx
                    }
    ...
    fn read_discrete_inputs(&mut self, addr: Address, cnt: Quantity) -> Result<Vec<Coil>> {
        self.core
            .block_on(async {
                tokio::select! {
                    ctx = self.async_ctx.read_discrete_inputs(addr, cnt) => {
                        ctx
                    }
                    _ = time::sleep(self.timeout) => {
                        Err(Error::new(ErrorKind::TimedOut,"timeout"))
    ...
    fn read_input_registers(&mut self, addr: Address, cnt: Quantity) -> Result<Vec<Word>> {
        self.core
            .block_on(async {
                tokio::select! {
                    ctx = self.async_ctx.read_input_registers(addr, cnt) => {
                        ctx
                    }
                    _ = time::sleep(self.timeout) => {
                        Err(Error::new(ErrorKind::TimedOut,"timeout"))
    ...
    fn read_holding_registers(&mut self, addr: Address, cnt: Quantity) -> Result<Vec<Word>> {
        self.core
            .block_on(async {
                tokio::select! {
                    ctx = self.async_ctx.read_holding_registers(addr, cnt) => {
                        ctx
                    }
                    _ = time::sleep(self.timeout) => {
                        Err(Error::new(ErrorKind::TimedOut,"timeout"))
...
impl Writer for Context {
    fn write_single_register(&mut self, addr: Address, data: Word) -> Result<()> {
        self.core.block_on(
            async {
                tokio::select! {
                        ctx = self.async_ctx.write_single_register(addr, data) => {
                            ctx
                        }
                        _ = time::sleep(self.timeout) => {
                            Err(Error::new(ErrorKind::TimedOut,"timeout"))
    ...
    fn write_multiple_registers(&mut self, addr: Address, data: &[Word]) -> Result<()> {
        self.core.block_on(
            async {
                tokio::select! {
                        ctx = self.async_ctx.write_multiple_registers(addr, data) => {
                            ctx
                        }
                        _ = time::sleep(self.timeout) => {
                            Err(Error::new(ErrorKind::TimedOut,"timeout"))
    ...
    fn write_single_coil(&mut self, addr: Address, coil: Coil) -> Result<()> {
        self.core.block_on(
            async {
                tokio::select! {
                        ctx = self.async_ctx.write_single_coil(addr, coil) => {
                            ctx
                        }
                        _ = time::sleep(self.timeout) => {
                            Err(Error::new(ErrorKind::TimedOut,"timeout"))
    ...
    fn write_multiple_coils(&mut self, addr: Address, coils: &[Coil]) -> Result<()> {
        self.core.block_on(
            async {
                tokio::select! {
                        ctx = self.async_ctx.write_multiple_coils(addr, coils) => {
                            ctx
                        }
                        _ = time::sleep(self.timeout) => {
                            Err(Error::new(ErrorKind::TimedOut,"timeout"))
