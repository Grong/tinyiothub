...
const TagFilter: FC<TagFilterProps> = ({
  ...
  const filteredTagList = useMemo(() => {
    const safeTagList = Array.isArray(tagList) ? tagList : []
    return safeTagList.filter(tag => tag.name.includes(searchKeywords))
  }, [tagList, searchKeywords])

  const currentTag = useMemo(() => {
    const safeTagList = Array.isArray(tagList) ? tagList : []
    return safeTagList.find(tag => tag.id === value[0])
  }, [value, tagList])
      ...
      <div className='relative'>
        ...
        >
          <div className={cn(
            ...
            <div className='p-[1px]'>
              <RiPriceTag3Line className='h-3.5 w-3.5 text-text-tertiary' />
            </div>
            <div className='text-[13px] leading-[18px] text-text-secondary'>
              {!value.length && t('tag.placeholder')}
            ...
            {!!value.length && (
              <div className='group/clear cursor-pointer p-[1px]' onClick={(e) => {
                ...
                <RiCloseCircleFill className='h-3.5 w-3.5 text-text-tertiary group-hover/clear:text-text-secondary' />
              </div>
            )}
          </div>
        </PortalToFollowElemTrigger>
          ...
          <div className='relative w-[240px] rounded-lg border-[0.5px] border-components-panel-border bg-components-panel-bg-blur shadow-lg backdrop-blur-[5px]'>
            ...
            <div className='max-h-72 overflow-auto p-1'>
              {filteredTagList.map(tag => (
                ...
                >
                  ...
                  {value.includes(tag.id) && <RiCheckLine className='h-4 w-4 shrink-0 text-text-secondary' />}
                </div>
              ))}
              {!filteredTagList.length && (
                <div className='flex flex-col items-center gap-1 p-3'>
                  <RiPriceTag3Fill className='h-6 w-6 text-text-tertiary' />
                  <div className='text-xs leading-[14px] text-text-tertiary'>{t('tag.noTag')}</div>
                </div>
              )}
            </div>
            ...
            <div className='p-1'>
              <div className='flex cursor-pointer select-none items-center gap-2 rounded-lg py-[6px] pl-3 pr-2 hover:bg-state-base-hover' onClick={() => {
                ...
                <RiPriceTag3Fill className='h-4 w-4 text-text-tertiary' />
