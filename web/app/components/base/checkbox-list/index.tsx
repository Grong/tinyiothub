          ...
          <div className='relative flex items-center gap-2 border-b border-divider-subtle px-3 py-2'>
            ...
            {!searchQuery ? <div className='flex min-w-0 flex-1 items-center gap-1'>
              ...
              {showCount && selectedCount > 0 && (
                <Badge uppercase>
                  {t('operation.selectCount', { count: selectedCount })}
                </Badge>
              )}
            </div> : <div className='system-sm-medium-uppercase flex-1 leading-6 text-text-secondary'>{
              filteredOptions.length > 0
                ? t('operation.searchCount', { count: filteredOptions.length, content: title })
                : t('operation.noSearchCount', { content: title })}</div>}
            {showSearch && (
              <SearchInput
                value={searchQuery}
          ...
          {!filteredOptions.length ? (
            <div className='px-3 py-6 text-center text-sm text-text-tertiary'>
              {searchQuery ? <div className='flex flex-col items-center justify-center gap-2'>
                ...
                <span className='system-sm-regular text-text-secondary'>{t('operation.noSearchResults', { content: title })}</span>
                <Button variant='secondary-accent' size='small' onClick={() => setSearchQuery('')}>{t('operation.resetKeywords')}</Button>
              </div> : t('noData')}
            </div>
          ) : (
