...
- **AI 处置流（大脑主干化 P0）**：报警产生即由 AI 值班员调查判断（噪声/可自愈/需人工），人只处理例外。Critical/Error 报警直达工单并附带 AI 并行调查结论；Warning/Info 进入 AI 分诊。
- **处置中心（新页面）**：AI 判断动态 feed——判断理由、证据摘录、审批倒计时、「对/错」反馈按钮；导航栏有待处理角标，离线回来后第一眼可见。
- **分级信任护栏**：影子期默认 annotate 模式（AI 只标注不动报警）；验证准确率后可按工作区翻开 suppress 模式（仅 Warning/Info 可被抑制，Critical/Error 永不）。kill switch 可整体回退到人工路径。
- **审批与执行闭环**：可自愈判断生成审批卡，批准后按白名单动作模板执行，执行验证（verified）通过才消警；未验证/执行异常自动转人工确认工单。审批 24h 未响应自动转工单。
- **判断质量基线**：24 场景 eval 套件（与生产同一 prompt 模板与解析器），每周 CI 定时跑（非阻断哨兵）。
- **/judgments API**：feed 列表（游标分页/48h 窗口/同设备规则折叠）、头部摘要（消化数/延迟 p50/p90/按类别反馈聚合）、反馈/批准/拒绝端点。
- **误判恢复**：点「错」并填原因 → 原因写入 AI 记忆（下次调查注入）、被抑制报警恢复活跃、判断自动重开重新调查。
- **演示种子脚本**：`scripts/demo_seed_dispositions.py` 一键重现「10 条报警进来」的演示场景（4 噪声/3 审批/3 工单）。

### Changed

- 报警不再唤醒心跳巡检，改为直达 thing-agent 调查派发（消除双路进 agent）；心跳回归主动巡检定位。
- 工单创建收敛为唯一入口（run 失败/需人工/审批拒绝/审批超时/Critical 直达五路共用，撞单自动折叠为复发事件）。
- 心跳唤醒的 ack 抑制与失败跳过语义不再作用于报警调查键域（复发故障不再被压 6 小时盲区）。

### Fixed

- AlarmStatus::Suppressed 补齐持久层（此前抑制状态无法落库、按抑制过滤静默返回全部）。
...
- 应用市场上架 3 个场景包模板（智慧楼层 / 智慧楼宇 / 智慧园区），marketplace 服务可直接浏览；模板详情完整返回场景参数与子设备结构（此前会被裁剪）
- 市场列表分页接受 `page_size`、驱动协议过滤接受 `protocol_type`（与前端实际发送的参数名对齐）
- 市场数据加载加固：同名条目去重告警、缺名字段丢弃、不合规文件加载期即告警且不计入列表总数、列表按名称稳定排序

### Fixed

- 市场分页与驱动协议过滤此前被静默忽略（永远固定 20 条/页）——现在真正生效
- 市场模板/驱动卡片的"下载数""协议"字段不再空白（前端字段名对齐后端契约）
- 云端拉取市场目录不再被第 1 页 20 条静默截断：自动翻页拉全量，带页数上限与降级信号保护（冷缓存响应明确报错而不是当空目录落库）
- 市场服务数据目录异常时不再呈现"健康的空目录"：该资源列表带降级头、详情返回 503、健康检查按资源报告 degraded 及真实原因（此前 1 小时后误报 degraded 且原因为写死的 rate_limit_exhausted）
- 云端市场代理未配置时返回明确"市场未启用"错误，不再静默请求公网地址；代理透传上游降级头、详情路径参数正确 URL 编码
- 驱动安装入口前置明确报错（动态加载未支持），不再走完下载流程才失败；checksum 测试豁免收窄为 debug 构建 + `sha256:test` 前缀（旧子串匹配过宽）
- 发布模板到只读市场服务时返回可操作的错误指引（405 翻译为中文指引）
- 极大页码分页参数不再触发整数溢出（debug panic / release 回绕）
- 测试基建：cloud 市场测试不再依赖公网真实服务（测试环境禁用 marketplace，行为确定性）；web 新增 `.nvmrc` 锁定 Node 20（Node 18 下 vitest 全线 ERR_REQUIRE_ESM 无法运行）
...
- **crates/agent 独立 crate**: agent 运行时（自治 loop、心跳、工具框架、session、prompt、memory 纯逻辑）从 cloud 抽取为可复用库，为边缘侧接入和 SDK 发布铺路；crate 零 axum/零 sqlx 依赖由 CI 强制
- **内存+事件持久化架构**: agent 状态全在内存运行，状态变化经事件幂等投影落库（乱序防护 + 断流全量重同步 + 5 分钟周期对账），运行速度不再受数据库写延迟约束
- **持久化自愈能力**: 落库订阅者崩溃自动重启并立即补同步；事件总线关停前排空；心跳结果带稳定 tick 身份，重试/重放/对账三路径幂等不重复
- **单实例保护**: 启动时对数据库旁路文件取排他锁，第二个进程响亮失败——杜绝双开导致的重复巡检和投影互覆
- **crates/authn 认证 crate**: JWT/密码机制独立成库，构造注入零全局态；HarmonyOS token 验证改常量时间比较
- **架构守卫 CI**: SQL 住所（全部 SQL 只住 crates/db）、迁移 DDL-only、crate 纯度（源码+依赖树+清单三层）四类守卫，并配故意违规自证测试——守卫本身坏了会立刻变红

### Changed

- **Db 门面统一数据访问**: 23 个 Repository 结构体解散为领域函数 + `impl Db` 薄委托，`AppState` 只持一个 `Arc<Db>`——一套数据一种入口，新数据访问的写法在 AGENTS.md 成文
- **迁移基线化**: 68 个历史迁移压缩为单 `baseline.sql`（与旧链终态逐表 schema diff 为空），迁移 runner 六层补丁全删；种子数据两级化（系统必需 + 演示开关）
- **启动快失败**: 启动期数据库读失败一律中止启动并指明原因（信任配置不再静默降级为宽松默认）；老链数据库的报错直接给出删库重建指引，新旧版本混用也能分清
- **信任配置热更新真实生效**: 运行中的心跳 loop 即时看到信任配置变更（此前要重启才生效——本次评审发现的生产 bug）
- **配置错误码修正**: 数据库故障时运行记录查询返回 500 而非误导性 404

### Fixed

- **FK 级联数据丢失根治**: 迁移在专用连接上以 FK OFF 运行——`DROP TABLE` 不再隐式级联清空子表（曾导致设备属性种子丢失、profile API 返回空）
- **静默失败通道封堵（评审驱动，23 项）**: 持久化订阅者崩溃从"无告警全停"变为可观测可恢复；信任配置事件带时间戳防护，旧事件无法覆盖新配置；重同步不再丢失问题去重元数据；损坏的信任配置行记 error 而非静默重置
- **认证路径 DoS 修复**: 畸形 HarmonyOS token（奇数长度/多字节字符）不再让请求任务 panic
- **测试可靠性**: 关键异步断言改为屏障式/快速失败轮询，消除慢 CI 上的抖动

### Removed

- 知识图谱与旧 device API 残留视图（knowledge/local-resources，被 Thing 本体取代）
- 进程级全局配置访问器、heartbeat/pending-action 全局态（全部改为构造注入）
- 失效的 Repository 实例群与迁移 runner 补丁层
