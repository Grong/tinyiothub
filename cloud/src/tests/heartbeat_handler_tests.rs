...
async fn test_get_heartbeat() {
    ...
    let response =
        app.oneshot(auth_request("GET", "/api/v1/heartbeat", &token, None)).await.unwrap();
...
async fn test_report_heartbeat() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("POST", "/api/v1/heartbeat", &token, Some(json!({}))))
        .await
        .unwrap();
...
async fn test_get_heartbeat_config() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response =
        app.oneshot(auth_request("GET", "/api/v1/heartbeat/config", &token, None)).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let (_s, json) = response_parts(response).await;
    assert!(json["code"].is_number());
}
