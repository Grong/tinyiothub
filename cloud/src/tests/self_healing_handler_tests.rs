...
async fn test_get_policies() {
    ...
    let response = app
        .oneshot(auth_request("GET", "/api/v1/self-healing/policies", &token, None))
        .await
        .unwrap();
...
async fn test_update_policies() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("PUT", "/api/v1/self-healing/policies", &token, Some(json!({}))))
        .await
        .unwrap();
...
async fn test_get_executions() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("GET", "/api/v1/self-healing/executions", &token, None))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let (_s, json) = response_parts(response).await;
...
async fn test_get_probes() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("GET", "/api/v1/self-healing/probes", &token, None))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let (_s, json) = response_parts(response).await;
...
async fn test_get_probe_config() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("GET", "/api/v1/self-healing/probes/config", &token, None))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let (_s, json) = response_parts(response).await;
...
async fn test_update_probe_config() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("PUT", "/api/v1/self-healing/probes/config", &token, Some(json!({}))))
        .await
        .unwrap();
    assert!(response.status().is_success() || response.status().is_client_error());
}
...
async fn test_execute_action_not_found() {
    ...
    let response = app
        .oneshot(auth_request(
            "POST",
            "/api/v1/self-healing/actions/nonexistent-level",
            &token,
            Some(json!({})),
        ))
        .await
