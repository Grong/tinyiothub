...
async fn test_create_role() {
    ...
    // Handler should return valid JSON with a code field
...
async fn test_create_role_duplicate_name() {
    ...
    // Try to create the same role twice — second should get an error
    let body = json!({ "name": "dup-role-test" });

    let response = app
        .clone()
        .oneshot(auth_request("POST", "/api/v1/users/roles", &token, Some(body.clone())))
        .await
        .unwrap();
    let (_status1, json1) = response_parts(response).await;
    ...
    let (status, json) = response_parts(response).await;
    assert_eq!(status, StatusCode::OK);
    // If first succeeded, second should fail with duplicate; if first failed, second may too
    assert!(json["code"].is_number(), "Response must have numeric code field");
}
...
async fn test_get_role_not_found() {
    ...
    let response = app
        .oneshot(auth_request("GET", "/api/v1/users/roles/nonexistent-id-12345", &token, None))
        .await
        .unwrap();
...
async fn test_update_role_not_found() {
    ...
    let response = app
        .oneshot(auth_request("PUT", "/api/v1/users/roles/nonexistent-id-12345", &token, Some(body)))
        .await
        .unwrap();
...
async fn test_delete_role_not_found() {
    ...
    let response = app
        .oneshot(auth_request("DELETE", "/api/v1/users/roles/nonexistent-id-12345", &token, None))
        .await
        .unwrap();
...
async fn test_get_role_permissions() {
    ...
    let response = app
        .oneshot(auth_request("GET", "/api/v1/users/roles/some-role-id/permissions", &token, None))
        .await
        .unwrap();

    let (status, json) = response_parts(response).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(json["code"], 0, "Expected success (stub implementation)");
...
async fn test_update_role_permissions() {
    ...
    let body = json!(["perm-1", "perm-2"]);

    let response = app
        .oneshot(auth_request("PUT", "/api/v1/users/roles/some-role-id/permissions", &token, Some(body)))
        .await
        .unwrap();
