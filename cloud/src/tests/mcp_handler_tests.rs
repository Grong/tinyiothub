...
async fn test_mcp_tools_list() {
    ...
    let response = app.oneshot(auth_request("POST", "/api/v1/mcp/tools/list", &token, Some(json!({})))).await.unwrap();
...
async fn test_mcp_tools_call_missing_params() {
    let app = setup_test_app().await;
    let token = create_test_token("user-1", "tenant-1");
    let response = app.oneshot(auth_request("POST", "/api/v1/mcp/tools/call", &token, Some(json!({})))).await.unwrap();
...
async fn test_mcp_request() {
    let app = setup_test_app().await;
    let token = create_test_token("user-1", "tenant-1");
    let response = app.oneshot(auth_request("POST", "/api/v1/mcp", &token, Some(json!({"jsonrpc": "2.0", "method": "initialize", "id": 1})))).await.unwrap();
