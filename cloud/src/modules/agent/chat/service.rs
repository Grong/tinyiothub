...
//
// Harness path: sends through AgentPool::send_message_streamed() for trust observation.
...
/// Convert AI-crate StreamEvent → ChatEvent for harness-based chat paths.
pub fn stream_event_to_chat_event(
    evt: &tinyiothub_ai::harness::StreamEvent,
    run_id: &str,
    session_key: &str,
) -> ChatEvent {
    match evt {
        tinyiothub_ai::harness::StreamEvent::Chunk { delta } => ChatEvent::Delta {
            run_id: run_id.to_string(),
            session_key: session_key.to_string(),
            message: serde_json::json!({
                "role": "assistant",
        ...
        tinyiothub_ai::harness::StreamEvent::Thinking { delta } => ChatEvent::Thinking {
            run_id: run_id.to_string(),
            session_key: session_key.to_string(),
            thinking: delta.clone(),
        },
        tinyiothub_ai::harness::StreamEvent::ToolCall { id: _, name, args } => {
            let args_str = serde_json::to_string(args).unwrap_or_default();
            ChatEvent::ToolCallStart {
                run_id: run_id.to_string(),
                session_key: session_key.to_string(),
        ...
        tinyiothub_ai::harness::StreamEvent::ToolResult {
            id: _,
            name,
            output,
            success,
        ...
        tinyiothub_ai::harness::StreamEvent::Final { text } => ChatEvent::Final {
            run_id: run_id.to_string(),
            session_key: session_key.to_string(),
            message: serde_json::json!({
                "role": "assistant",
        ...
        tinyiothub_ai::harness::StreamEvent::Error { message } => ChatEvent::Error {
            run_id: run_id.to_string(),
            session_key: session_key.to_string(),
            error: message.clone(),
        },
...
/// Send a chat message through the harness streaming path.
///
/// Differs from `send_message` in that it uses `AgentPoolLike::send_message_streamed()`
/// (StreamEvent) instead of raw zeroclaw TurnEvent. This gives the harness visibility
/// into tool calls for trust checking and verification.
#[allow(clippy::too_many_arguments)]
pub async fn send_message_harness(
    agent_pool: &Arc<dyn tinyiothub_ai::agent::pool::AgentPoolLike>,
    ...
    let reflection_model = model.to_string();
    let event_publisher = event_publisher.clone();
    let agent_pool = Arc::clone(agent_pool);
    let zeroclaw_agent = Arc::clone(zeroclaw_agent);
    ...
    let run_id_for_remove = run_id.clone();
    let handle = tokio::spawn(async move {
        // Set system prompt on zeroclaw agent
        {
            let mut ag = zeroclaw_agent.lock().await;
            if ag.history().is_empty() && !system_prompt.is_empty() {
                ag.seed_history(&[zeroclaw::providers::traits::ChatMessage {
                    role: "system".into(),
                    content: system_prompt,
        ...
        // Use harness streaming path (StreamEvent instead of TurnEvent)
        let stream_result = agent_pool.send_message_streamed(&workspace_id, &message).await;
        ...
        let final_text = match stream_result {
            Ok(mut stream_rx) => {
                ...
                while let Some(evt) = stream_rx.recv().await {
                    ...
                    if let tinyiothub_ai::harness::StreamEvent::Final { text: ref t } = evt {
                        final_output = Some(t.clone());
                    }

                    let chat_event =
                        stream_event_to_chat_event(&evt, &run_id, &session_key);
                    if tx.send(chat_event).await.is_err() {
                        break;
                    }
                }

                final_output.or(if text.is_empty() { None } else { Some(text) })
            }
            Err(e) => {
                let _ = tx
                    .send(ChatEvent::Error {
