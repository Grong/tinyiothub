...
impl DevicePerformanceService {
    pub fn new(database: Arc<Database>, device_cache: Arc<DeviceCache>, alarm_repository: Arc<AlarmRepositoryImpl>) -> Self {
        let monitoring_service = DeviceMonitoringService::new(database.clone(), device_cache.clone(), alarm_repository.clone());
    ...
    pub async fn get_device_performance_metrics(&self, device_id: &str) -> Option<DevicePerformanceMetrics> {
        if let Some(device) = self.device_cache.get(device_id) {
            let mut metrics = DevicePerformanceMetrics {
                ...
                cpu_usage: None, memory_usage: None, network_latency_ms: None,
                response_time_ms: None, throughput_ops_per_sec: None,
                error_rate: None, uptime_percentage: None,
            ...
            if let Some(stats) = self.monitoring_service.get_device_metrics(device_id).await {
                ...
                if stats.online_properties > 0 {
                    ...
                    let quality_factor = self.monitoring_service.get_device_connection_quality(device_id)
                        .map(|q| q as f64 / 100.0).unwrap_or(0.5);
        ...
        if let Some(device) = self.device_cache.get(device_id)
            && let Some(created_at) = &device.created_at
                && let Ok(created_time) = chrono::DateTime::parse_from_str(
                    &format!("{} +00:00", created_at), "%Y-%m-%d %H:%M:%S %z",
                ) {
                    let now = chrono::Utc::now();
    ...
    async fn estimate_device_resource_usage(&self, device_id: &str) -> (Option<f64>, Option<f64>) {
        if let Some(device) = self.device_cache.get(device_id) {
            ...
            let online_factor = if self.monitoring_service.is_device_online(device_id) { 1.2 } else { 0.3 };
            let quality_factor = self.monitoring_service.get_device_connection_quality(device_id)
                .map(|q| 0.5 + (q as f64 / 100.0) * 0.5).unwrap_or(0.7);
            let cpu = ((base_cpu + property_count * 0.5 + command_count * 0.3) * online_factor * quality_factor).clamp(1.0, 95.0);
            let mem = ((base_memory + property_count * 1.0 + command_count * 0.5) * online_factor * quality_factor).clamp(5.0, 90.0);
            (Some(cpu), Some(mem))
        } else {
            (None, None)
        }
    ...
    pub async fn get_device_performance_history(&self, device_id: &str, hours: u32) -> Result<Vec<DevicePerformanceMetrics>, Error> {
        if self.device_cache.get(device_id).is_none() { return Err(Error::NotFound); }
        ...
        for i in 0..hours {
            ...
            if let Some(mut metrics) = self.get_device_performance_metrics(device_id).await {
                ...
                if let Some(cpu) = metrics.cpu_usage { metrics.cpu_usage = Some((cpu * variation_factor).clamp(0.0, 100.0)); }
                if let Some(memory) = metrics.memory_usage { metrics.memory_usage = Some((memory * variation_factor).clamp(0.0, 100.0)); }
                if let Some(latency) = metrics.network_latency_ms { metrics.network_latency_ms = Some((latency * (2.0 - variation_factor)).max(1.0)); }
    ...
    pub async fn check_device_performance_alerts(&self, device_id: &str) -> Vec<PerformanceAlert> {
        ...
        if let Some(metrics) = self.get_device_performance_metrics(device_id).await {
            if let Some(cpu) = metrics.cpu_usage {
                if cpu > 90.0 { alerts.push(PerformanceAlert::new(device_id, "high_cpu", "critical", &format!("CPU usage critical: {:.1}%", cpu), cpu, 90.0)); }
                else if cpu > 80.0 { alerts.push(PerformanceAlert::new(device_id, "high_cpu", "warning", &format!("CPU usage high: {:.1}%", cpu), cpu, 80.0)); }
            }
            if let Some(mem) = metrics.memory_usage {
                if mem > 95.0 { alerts.push(PerformanceAlert::new(device_id, "high_memory", "critical", &format!("Memory usage critical: {:.1}%", mem), mem, 95.0)); }
                else if mem > 85.0 { alerts.push(PerformanceAlert::new(device_id, "high_memory", "warning", &format!("Memory usage high: {:.1}%", mem), mem, 85.0)); }
            }
            if let Some(latency) = metrics.network_latency_ms {
                if latency > 200.0 { alerts.push(PerformanceAlert::new(device_id, "high_latency", "critical", &format!("Latency critical: {:.1}ms", latency), latency, 200.0)); }
                else if latency > 100.0 { alerts.push(PerformanceAlert::new(device_id, "high_latency", "warning", &format!("Latency high: {:.1}ms", latency), latency, 100.0)); }
            }
            if let Some(rt) = metrics.response_time_ms {
                if rt > 5000.0 { alerts.push(PerformanceAlert::new(device_id, "slow_response", "critical", &format!("Response time critical: {:.1}ms", rt), rt, 5000.0)); }
                else if rt > 3000.0 { alerts.push(PerformanceAlert::new(device_id, "slow_response", "warning", &format!("Response time high: {:.1}ms", rt), rt, 3000.0)); }
            }
            if let Some(er) = metrics.error_rate {
                if er > 0.1 { alerts.push(PerformanceAlert::new(device_id, "high_error_rate", "critical", &format!("Error rate critical: {:.1}%", er * 100.0), er * 100.0, 10.0)); }
                else if er > 0.05 { alerts.push(PerformanceAlert::new(device_id, "high_error_rate", "warning", &format!("Error rate high: {:.1}%", er * 100.0), er * 100.0, 5.0)); }
            }
            if let Some(up) = metrics.uptime_percentage {
                if up < 90.0 { alerts.push(PerformanceAlert::new(device_id, "low_uptime", "critical", &format!("Uptime critical: {:.1}%", up), up, 90.0)); }
                else if up < 95.0 { alerts.push(PerformanceAlert::new(device_id, "low_uptime", "warning", &format!("Uptime low: {:.1}%", up), up, 95.0)); }
            }
        }
        alerts
    }
    ...
    pub async fn get_system_performance_overview(&self) -> SystemPerformanceOverview {
        ...
        for device in &all_devices {
            if let Some(metrics) = self.get_device_performance_metrics(&device.id).await {
                ...
                if let Some(cpu) = metrics.cpu_usage { total_cpu += cpu; if cpu > 80.0 { high_cpu += 1; } }
                if let Some(mem) = metrics.memory_usage { total_mem += mem; if mem > 85.0 { high_mem += 1; } }
                if let Some(lat) = metrics.network_latency_ms { total_latency += lat; if lat > 100.0 { high_latency += 1; } }
                if let Some(tp) = metrics.throughput_ops_per_sec { total_throughput += tp; }
            }
        }
        let avg = |total: f64, count: u32| if count > 0 { Some(total / count as f64) } else { None };
        SystemPerformanceOverview {
            total_devices, devices_with_metrics,
            ...
            high_cpu_devices: high_cpu, high_memory_devices: high_mem, high_latency_devices: high_latency,
            last_updated: chrono::Utc::now().format("%Y-%m-%d %H:%M:%S").to_string(),
        }
    }
}
...
impl PerformanceAlert {
    pub fn new(device_id: &str, alert_type: &str, severity: &str, message: &str, current_value: f64, threshold: f64) -> Self {
        Self {
            device_id: device_id.to_string(), alert_type: alert_type.to_string(),
            severity: severity.to_string(), message: message.to_string(),
