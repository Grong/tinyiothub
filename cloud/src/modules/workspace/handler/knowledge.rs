...
/// Upload a text file and create a knowledge document from its content.
pub async fn upload_file(
    ...
    if !found_file || content.is_empty() {
        return ApiResponseBuilder::error_with_code(400, "未上传有效文件或文件为空");
    }
    ...
    match state
        .knowledge_service
        .create_document(&id, title, content, tags)
        .await
    {
        Ok(doc) => ApiResponseBuilder::success(doc),
        Err(e) => {
            tracing::error!("Failed to create document from upload: {}", e);
            ApiResponseBuilder::error("上传文件失败")
        }
    }
}

/// POST /{id}/knowledge/files/upload-raw
/// Upload a file as raw bytes (simpler alternative to multipart).
/// Accepts octet-stream body with X-File-Name and X-Tags headers.
pub async fn upload_file_raw(
    ...
    if body.is_empty() {
        return ApiResponseBuilder::error_with_code(400, "文件内容为空");
    }
