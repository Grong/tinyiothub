...
pub fn register_builtin_drivers(registry: &crate::modules::plugin::PluginRegistry) {
    use crate::modules::device::driver::{get_driver_list, get_supported_driver_names};
    ...
    for (info, name) in driver_infos.into_iter().zip(driver_names) {
        ...
        }) as crate::modules::plugin::PluginFactory;
...
pub fn init_plugins(_context: Arc<AppContext>) -> Result<(), Error> {
    let registry = crate::modules::plugin::get_global_registry();
