...
pub use handlers::{HttpPollHandler, MqttSubscribeHandler, ProtocolHandler};

use crate::{modules::plugin::PluginHandler, shared::error::Error};

/// 创建协议处理器
pub fn create_handler(config: &toml::Value) -> Result<Box<dyn PluginHandler>, Error> {
    let protocol_cfg = config
        .get("protocol")
        .ok_or_else(|| Error::ValidationError("Missing [protocol] section".to_string()))?;

    match protocol_cfg.get("type").and_then(|v| v.as_str()) {
        Some("http_poll") => {
            let mut json_val: serde_json::Value = protocol_cfg
                .clone()
                .try_into()
