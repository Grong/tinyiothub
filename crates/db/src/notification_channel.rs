...
/// 按类型查询启用的通知渠道（id, name, config；自 cloud alarm NotificationDispatcher 吸收，Task 11，SQL 逐字）
pub async fn find_enabled_notification_channels_by_type(
    db: &Db,
    channel_type: &str,
    workspace_id: &str,
    ...
    Ok(rows
        .iter()
        .map(|row| (row.get("id"), row.get("name"), row.get("config")))
        .collect())
}
