...
impl PermissionRepository {
    ...
    pub async fn update(&self, id: &str, request: &UpdatePermissionRequest) -> Result<Permission> {
    ...
    pub async fn find_all(&self, params: &PermissionQuery) -> Result<Vec<Permission>> {
    ...
    pub async fn count(&self, params: &PermissionQuery) -> Result<i64> {
    ...
    pub async fn find_by_resource_type(&self, resource_type: &str) -> Result<Vec<Permission>> {
    ...
    pub async fn find_by_action_type(&self, action_type: &str) -> Result<Vec<Permission>> {
        let rows = sqlx::query_as::<_, PermissionRow>(
    ...
    pub async fn find_by_parent_id(&self, parent_id: &str) -> Result<Vec<Permission>> {
        let rows = sqlx::query_as::<_, PermissionRow>(
    ...
    pub async fn exists_by_code_exclude_id(&self, code: &str, exclude_id: &str) -> Result<bool> {
    ...
    pub async fn find_by_ids(&self, ids: &[String]) -> Result<Vec<Permission>> {
        if ids.is_empty() {
            return Ok(vec![]);
        }
...
impl PermissionGroupRepository {
    pub async fn find_by_id(&self, id: &str) -> Result<Option<PermissionGroup>> {
    ...
    pub async fn find_by_name(&self, name: &str) -> Result<Option<PermissionGroup>> {
        let row = sqlx::query_as::<_, PermissionGroupRow>(
    ...
    pub async fn create(&self, request: &CreatePermissionGroupRequest) -> Result<PermissionGroup> {
        let id = uuid::Uuid::new_v4().to_string();
        let now = chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
        let permissions_json = serde_json::to_string(&request.permission_ids).unwrap_or_else(|_| "[]".to_string());
    ...
    pub async fn find_all(&self) -> Result<Vec<PermissionGroup>> {
