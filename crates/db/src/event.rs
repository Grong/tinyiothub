...
impl EventRepository {
    ...
    pub async fn find_by_criteria(&self, criteria: &EventCriteria) -> Result<Vec<Event>> {
        // Build base SQL
    ...
    pub async fn count_by_level(&self, level: EventLevel) -> Result<u64> {
    ...
    pub async fn count_by_type(&self, event_type: &EventType) -> Result<u64> {
    ...
    pub async fn save_batch(&self, events: &[Event]) -> Result<()> {
        if events.is_empty() {
            return Ok(());
        }
    ...
    pub async fn get_statistics(&self, _params: &StatisticsParams) -> Result<EventStatistics> {
    ...
    pub async fn export_events(&self, criteria: &EventCriteria, format: ExportFormat) -> Result<Vec<u8>> {
        ...
        match format {
            ...
            ExportFormat::Excel => {
                ...
                Box::pin(self.export_events(criteria, ExportFormat::Csv)).await
            }
        }
    }
}
...
impl RealTimeEventRepository {
    pub async fn upsert_status(&self, event: &Event) -> Result<()> {
    ...
    pub async fn remove_status(&self, source: &EventSource, event_type: &EventType) -> Result<()> {
    ...
    pub async fn find_active_events(&self, filter: &RealTimeFilter) -> Result<Vec<RealTimeEvent>> {
    ...
    pub async fn get_status_summary(&self, _filter: &RealTimeFilter) -> Result<StatusSummary> {
...
impl RealTimeEventRepository {
    ...
    /// input).
    pub async fn execute_active_events_query(&self, filter: &RealTimeFilter) -> Result<Vec<sqlx::sqlite::SqliteRow>> {
