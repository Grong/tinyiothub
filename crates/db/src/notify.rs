...
fn row_to_notification_rule(row: &sqlx::sqlite::SqliteRow) -> Result<NotificationRule> {
...
pub(crate) async fn get_rules_by_method(
    ...
    for row in rows {
        match row_to_notification_rule(&row) {
            Ok(rule) => {
...
pub(crate) async fn set_rule_enabled(pool: &SqlitePool, rule_id: &str, enabled: bool) -> Result<()> {
...
/// Get rule statistics
pub(crate) async fn get_rule_statistics(pool: &SqlitePool) -> Result<RuleStatistics> {
    let row = sqlx::query(
        r#"
        SELECT
...
pub(crate) async fn create_rule(pool: &SqlitePool, rule: &NotificationRule) -> Result<()> {
...
pub(crate) async fn get_rule(pool: &SqlitePool, rule_id: &str) -> Result<Option<NotificationRule>> {
    let row = sqlx::query(
        r#"
        SELECT id, name, description, event_type, event_subtype, event_level,
               device_filter, notification_methods, recipients, enabled, created_at, updated_at, workspace_id
    ...
    if let Some(row) = row {
        let rule = row_to_notification_rule(&row)?;
...
pub(crate) async fn list_enabled_rules(pool: &SqlitePool) -> Result<Vec<NotificationRule>> {
    let rows = sqlx::query(
        r#"
        SELECT id, name, description, event_type, event_subtype, event_level,
               device_filter, notification_methods, recipients, enabled, created_at, updated_at, workspace_id
...
pub(crate) async fn update_rule(pool: &SqlitePool, rule: &NotificationRule) -> Result<()> {
    let notification_methods_json =
        serde_json::to_string(&rule.notification_methods.iter().map(|m| m.as_str()).collect::<Vec<_>>())?;
    let recipients_json = serde_json::to_string(&rule.recipients)?;
    let device_filter_json = if let Some(ref filter) = rule.device_filter {
...
pub(crate) async fn delete_rule(pool: &SqlitePool, rule_id: &str) -> Result<()> {
...
pub(crate) async fn get_rules_by_event_type(
    pool: &SqlitePool,
    event_type: &str,
...
pub(crate) async fn get_records_by_rule(pool: &SqlitePool, rule_id: &str) -> Result<Vec<NotificationRecord>> {
    let rows = sqlx::query(
        r#"
...
pub(crate) async fn get_history_statistics(pool: &SqlitePool, days: i32) -> Result<HistoryStatistics> {
...
pub(crate) async fn cleanup_old_records(pool: &SqlitePool, days: i32) -> Result<u64> {
    let cutoff_date = (Utc::now() - chrono::Duration::days(days as i64)).to_rfc3339();
...
pub(crate) async fn store_record(pool: &SqlitePool, record: &NotificationRecord) -> Result<()> {
...
pub(crate) async fn get_records(pool: &SqlitePool, event_id: &str) -> Result<Vec<NotificationRecord>> {
    let rows = sqlx::query(
        r#"
        SELECT id, event_id, rule_id, notification_method, recipient,
               status, sent_at, error_message, created_at
...
pub(crate) async fn update_record_status(
    pool: &SqlitePool,
    record_id: &str,
    status: NotificationStatus,
...
impl Db {
    ...
    /// 启用/禁用规则。
    pub async fn set_notification_rule_enabled(&self, rule_id: &str, enabled: bool) -> Result<()> {
        set_rule_enabled(self.pool(), rule_id, enabled).await
    }

    /// 规则统计。
    pub async fn get_notification_rule_statistics(&self) -> Result<RuleStatistics> {
        get_rule_statistics(self.pool()).await
    }

    /// 创建规则。
    pub async fn create_notification_rule(&self, rule: &NotificationRule) -> Result<()> {
        create_rule(self.pool(), rule).await
    }

    /// 按 ID 查询规则。
    pub async fn get_notification_rule(&self, rule_id: &str) -> Result<Option<NotificationRule>> {
        get_rule(self.pool(), rule_id).await
    }

    /// 全部规则。
    pub async fn list_notification_rules(&self) -> Result<Vec<NotificationRule>> {
        list_rules(self.pool()).await
    }

    /// 启用的规则。
    pub async fn list_enabled_notification_rules(&self) -> Result<Vec<NotificationRule>> {
        list_enabled_rules(self.pool()).await
    }

    /// 更新规则。
    pub async fn update_notification_rule(&self, rule: &NotificationRule) -> Result<()> {
        update_rule(self.pool(), rule).await
    }

    /// 删除规则。
    pub async fn delete_notification_rule(&self, rule_id: &str) -> Result<()> {
        delete_rule(self.pool(), rule_id).await
    }

    /// 按事件类型查询启用规则。
    pub async fn get_notification_rules_by_event_type(
        &self,
        event_type: &str,
        event_subtype: Option<&str>,
    ) -> Result<Vec<NotificationRule>> {
        get_rules_by_event_type(self.pool(), event_type, event_subtype).await
    }

    /// 按规则查询投递记录。
    pub async fn get_notification_records_by_rule(&self, rule_id: &str) -> Result<Vec<NotificationRecord>> {
        get_records_by_rule(self.pool(), rule_id).await
    }
    ...
    /// 投递统计。
    pub async fn get_notification_history_statistics(&self, days: i32) -> Result<HistoryStatistics> {
        get_history_statistics(self.pool(), days).await
    }

    /// 清理旧投递记录。
    pub async fn cleanup_old_notification_records(&self, days: i32) -> Result<u64> {
        cleanup_old_records(self.pool(), days).await
    }

    /// 分页查询投递记录。
    pub async fn get_notification_records_paginated(
        &self,
        offset: u32,
        limit: u32,
    ) -> Result<(Vec<NotificationRecord>, u64)> {
        get_records_paginated(self.pool(), offset, limit).await
    }

    /// 写入投递记录。
    pub async fn store_notification_record(&self, record: &NotificationRecord) -> Result<()> {
        store_record(self.pool(), record).await
    }

    /// 按事件查询投递记录。
    pub async fn get_notification_records(&self, event_id: &str) -> Result<Vec<NotificationRecord>> {
        get_records(self.pool(), event_id).await
    }

    /// 更新投递记录状态。
    pub async fn update_notification_record_status(
        &self,
        record_id: &str,
        status: NotificationStatus,
        error_message: Option<String>,
    ) -> Result<()> {
        update_record_status(self.pool(), record_id, status, error_message).await
    }
}
