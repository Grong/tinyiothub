...
async fn baseline_pool_with_seed_data() -> (SqlitePool, std::path::PathBuf) {
    ...
    let path = std::env::temp_dir().join(format!("thing-rename-data-{}-{seq}.db", std::process::id()));
    ...
    sqlx::query(
        "INSERT INTO events (id, event_level, event_type, event_subtype, timestamp, source_type, title, device_id)
    ...
    sqlx::query(
        "INSERT INTO messages (id, level, title, device_type, device_id)
    ...
    .unwrap();
    sqlx::query(
        "INSERT INTO agent_memories (id, workspace_id, agent_id, content, device_id)
    ...
    .unwrap();
    sqlx::query(
        "INSERT INTO agent_actions (id, workspace_id, agent_id, event_type, action_type, content, device_id)
