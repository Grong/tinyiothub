...
async fn fresh_migrated_pool() -> (SqlitePool, std::path::PathBuf) {
    ...
    let path = std::env::temp_dir().join(format!("thing-rename-schema-{}-{seq}.db", std::process::id()));
...
async fn device_type_and_limit_renamed() {
    ...
    for t in ["things", "thing_templates"] {
        ...
        assert!(
            cols.contains(&"thing_type".to_string()) || t == "thing_templates",
    ...
    assert!(
        !msgs.contains(&"device_type".to_string()),
        "messages still has device_type"
    );
    let _ = std::fs::remove_file(path);
}
...
async fn tags_check_and_trigger_renamed() {
    let (pool, path) = fresh_migrated_pool().await;
    let tags_sql: String = sqlx::query_scalar("SELECT sql FROM sqlite_master WHERE type='table' AND name='tags'")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert!(
        !tags_sql.contains("'device'"),
        "tags CHECK still allows 'device': {tags_sql}"
    );
    let trigger: Option<String> =
        sqlx::query_scalar("SELECT name FROM sqlite_master WHERE type='trigger' AND name='keep_thing_memory_limit'")
            .fetch_optional(&pool)
            .await
            .unwrap();
    assert!(trigger.is_some(), "keep_thing_memory_limit missing");
    let old_trigger: Option<String> =
        sqlx::query_scalar("SELECT name FROM sqlite_master WHERE type='trigger' AND name='keep_device_memory_limit'")
            .fetch_optional(&pool)
            .await
            .unwrap();
