...
mod tests {
    ...
    #[test]
    fn test_registry_register_custom_executor() {
        // Verify the registry accepts externally-defined executors (this is how
        // cloud wires the db-bound executors from tinyiothub_runtime).
        struct Stub;
        #[async_trait]
        impl JobExecutor for Stub {
            fn can_handle(&self, job_type: &str) -> bool {
                job_type == "stub"
            }
            async fn execute(
        ...
        assert!(registry.find("stub").is_none());
