...
/// Admin domain state slice (P4-Task24) — the fields of cloud's `AppState`
/// the admin handlers actually consume. The composition layer (cloud)
...
pub struct AdminState {
    ...
    /// 网络默认配置切片（/system/network）—— FromRef 时从进程全局配置克隆，
    /// 与原先每次请求 `config::get()` 读取语义一致（配置启动时设置一次）。
...
impl AdminState {
    ...
    ///
    /// AppState 同名方法的域内移植（P4-Task24）：workspace_id 为 None 时记录
    /// 安全警告并使用空 workspace（查不到任何设备），绝不回退到未隔离的
    /// 原始仓库。
    ...
    ///
    /// AppState 同名方法的域内移植（P4-Task24）：显式 workspace_id 直接返回，
    /// 否则查询该租户的默认工作空间。
    ...
    /// 更新设备属性值
    ///
    /// AppState 同名方法的域内移植（P4-Task24）：验证 + 发布 PropertyChange
