...
pub async fn deliver(report: &RunReport, signal: &WakeSignal, registry: &RunRegistry, host: &dyn ThingAgentHost) {
...
mod tests {
    ...
    use crate::runtime::thing_agent::types::{ActionRecord, Priority};
    use std::sync::Mutex;
    ...
    async fn runner_rejected_outcome_reaches_relax_hint_end_to_end() {
        use crate::runtime::thing_agent::runner::{ToolTraceEntry, build_actions, decide_outcome};
