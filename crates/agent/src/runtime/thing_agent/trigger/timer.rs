...
use super::Trigger;
use crate::runtime::thing_agent::traits::AutonomyPolicyReader;
...
mod tests {
    ...
    /// 读取故障桩 —— load_autonomy 必失败，等效原空库 broken_repo 的故障注入。
    fn broken_repo() -> Arc<StubAutonomyPolicyReader> {
        let stub = Arc::new(StubAutonomyPolicyReader::new());
        stub.set_fail(true);
        stub
    }
    ...
    fn spawn_trigger(
        workspace_id: &str,
        interval: Duration,
        repo: Arc<dyn AutonomyPolicyReader>,
    ...
    async fn emits_one_signal_per_interval_with_timer_dedup_key() {
        ...
        let repo = stub_repo("ws_01", AutonomyMode::Act);
    ...
    async fn mode_off_emits_zero_signals_until_policy_changes() {
        ...
        let repo = stub_repo("ws_off", AutonomyMode::Off);
        ...
        repo.save("ws_off", policy(AutonomyMode::Act));
    ...
    async fn diagnose_mode_emits_signals() {
        ...
        let repo = stub_repo("ws_diag", AutonomyMode::Diagnose);
    ...
    async fn policy_read_failure_emits_no_signals() {
        ...
        drop(rx);
        handle.abort();
        let repo = stub_repo("ws_err", AutonomyMode::Act);
        let (mut rx, handle) = spawn_trigger("ws_err", Duration::from_secs(60), repo);
    ...
    async fn run_returns_ok_when_receiver_dropped() {
        ...
        let repo = stub_repo("ws_02", AutonomyMode::Act);
        let trigger = TimerTrigger {
