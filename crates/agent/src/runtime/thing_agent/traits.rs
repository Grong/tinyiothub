...
///
/// Value type lives in `tinyiothub_core::models::event`（Task 13 归位 core：
/// event 管道生产、本 crate 消费的共享契约）；此处随 trait 一同导出。
///
...
/// 工作区自治策略读取端口（Task 13：斩断 runtime → 存储实现边）。
///
/// 运行时需要按工作区读取自治策略（mode/配额）做 gate；具体持久化实现住
/// apps/cloud（适配 cloud 组合的 repository），本 crate 只面向本端口编程。
#[async_trait::async_trait]
pub trait AutonomyPolicyReader: Send + Sync {
    /// 读取工作区自治策略；无配置返回 `Ok(None)`。
...
/// SchedulerHandle；[`crate::runtime::thing_agent::scheduler::SchedulerHandle`]
/// 本身也实现它（单工作区直连，测试/桩用）。
#[async_trait::async_trait]
pub trait DirectiveSink: Send + Sync {
    /// 投递唤醒信号。语义与
    /// [`crate::runtime::thing_agent::scheduler::SchedulerHandle::enqueue`] 相同：
    /// 队列满拒绝用户指令（Rejected）、60s 同文去重（Duplicate）等
    /// 由实现方保证。
    fn enqueue(
        &self,
        signal: crate::runtime::thing_agent::types::WakeSignal,
    ) -> Result<(), crate::runtime::thing_agent::scheduler::EnqueueError>;

    /// O26 kill switch：清空该工作区调度器的待处理队列（ready queue + 未
    /// flush 的合并窗口）。语义与
    /// [`crate::runtime::thing_agent::scheduler::SchedulerHandle::drain`] 相同：不取消
...
pub(crate) mod test_stubs {
    //! 共享测试桩：内存版 [`AutonomyPolicyReader`]（Task 13 起替代真实
    //! SQLite repo 夹具 —— 本 crate 全树禁存储依赖，SQL 行为由 db crate
    //! 自测覆盖）。`testing` feature 下对 crate 外消费方（cloud 集成测试
    //! 夹具）可用。
    ...
    /// 内存策略桩：`save` 播种/翻牌（等效原 save_autonomy），`set_fail`
    /// 注入读取故障（fail-closed 路径）。
    pub(crate) struct StubAutonomyPolicyReader {
        policies: Mutex<HashMap<String, AutonomyPolicy>>,
        fail: Mutex<bool>,
    }
    ...
    impl StubAutonomyPolicyReader {
        ...
        pub(crate) fn with_policy(workspace_id: &str, mode: AutonomyMode) -> Self {
            let stub = Self::new();
            stub.save(workspace_id, policy(mode));
            stub
        }

        pub(crate) fn save(&self, workspace_id: &str, policy: AutonomyPolicy) {
            self.policies
                .lock()
    ...
    #[async_trait::async_trait]
    impl super::AutonomyPolicyReader for StubAutonomyPolicyReader {
        async fn load_autonomy(&self, workspace_id: &str) -> anyhow::Result<Option<AutonomyPolicy>> {
            if *self.fail.lock().expect("fail lock") {
                anyhow::bail!("stub autonomy policy read failure");
            }
            Ok(self.policies.lock().expect("policies lock").get(workspace_id).cloned())
        }
    }
}
...
mod tests {
    ...
    async fn scheduler_handle_is_a_directive_sink() {
        use crate::runtime::thing_agent::scheduler::Scheduler;
        ...
        assert_eq!(
            dup,
            Err(crate::runtime::thing_agent::scheduler::EnqueueError::Duplicate)
        );
    }
}
