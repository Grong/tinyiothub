...
use crate::host::shared::config::{
    AgentError, AgentRuntimeConfig, compute_hash, default_agent_config,
};
...
pub async fn get_config(
    ...
    let row: Option<(String,)> =
        sqlx::query_as("SELECT config FROM agent_configs WHERE agent_id = ?")
            .bind(agent_id)
...
pub async fn get_config_json(
    ...
    if let Some((config_str, config_hash)) = row {
        let config: serde_json::Value =
            serde_json::from_str(&config_str).unwrap_or_else(|_| default_agent_config());
