...
use crate::plugin::{AppContext, PluginHandler, PluginManifest, PluginType};
...
/// 注册内置驱动到插件注册表
pub fn register_builtin_drivers(registry: &crate::plugin::PluginRegistry) {
...
pub fn init_plugins(_context: Arc<AppContext>) -> Result<(), Error> {
    let registry = crate::plugin::get_global_registry();
