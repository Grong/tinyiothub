...
impl PluginRegistry {
    ...
    fn create_config_plugin(
        ...
        match manifest.plugin_type {
            PluginType::Protocol => {
                let handler = crate::plugin::protocol::create_handler(config)?;
                Ok(handler)
            }
            ...
            PluginType::Scheduler => {
                let handler = crate::plugin::scheduler::create_handler(config)?;
                Ok(handler)
            }
            PluginType::Storage => {
                let handler = crate::plugin::storage::create_handler(config, context)?;
                Ok(handler)
            }
            PluginType::Integration => {
                let handler = crate::plugin::integration::create_handler(config, context)?;
                Ok(handler)
            }
        }
    }
