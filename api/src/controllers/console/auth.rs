use crate::{
    ...
    views::{api_result::ApiResult, auth::{CurrentResponse, LoginResponse}},
};
...
async fn register(
    ...
    // 跳过验证，直接注册成功.
    verify(State(ctx), Path(user.email_verification_token.unwrap())).await?;
    format::json(())
}
...
async fn login(State(ctx): State<AppContext>, Json(params): Json<LoginParams>) -> Result<Response> {
    ...
    format::json(ApiResult::success(Some(LoginResponse::new(&user, &token))))
}
