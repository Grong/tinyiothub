...
/// Resolve workspace_id from explicit param or tenant's first workspace.
...
fn map_create_request(req: &CreateJobRequest) -> CreateCronJobRequest {
    ...
    // For device_command, pack target fields into config JSON
...
fn map_update_request(req: &UpdateJobRequest) -> UpdateCronJobRequest {
    ...
    // If config is being updated and this is a device_command, repack
...
// ─── DTO mapping: legacy JobExecution <-> CronRun ─────────────────────────
