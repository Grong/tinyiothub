...
async fn get_wechat_qrcode(
    ...
    // 生成 state
    let state = params.state.unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
...
/// 获取社交登录配置
async fn get_social_config(
    State(state): State<AppState>,
) -> Json<ApiResponse<Vec<SocialConfig>>> {
    let db = state.database();
    
    let sql = "SELECT provider, app_id, app_secret, redirect_uri, is_enabled FROM social_configs";
    
    let rows = match db.query(sql, |row| {
        Ok(SocialConfig {
