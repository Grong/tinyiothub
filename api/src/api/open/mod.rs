...
async fn list_devices(
    ...
    let rows = state.database.query(&sql, |row| {
        Ok(serde_json::json!({
    ...
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", "/open/devices", StatusCode::OK, latency_ms).await;
...
async fn get_device(
    ...
    let mut rows = state.database.query(&sql, |row| {
        Ok(serde_json::json!({
            "id": row.try_get::<String, _>("id")?,
            "name": row.try_get::<String, _>("name")?,
            "display_name": row.try_get::<Option<String>, _>("display_name")?,
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", &format!("/open/devices/{}", id), StatusCode::OK, latency_ms).await;
...
async fn get_device_properties(
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", &format!("/open/devices/{}/properties", id), StatusCode::OK, latency_ms).await;
...
async fn list_commands(
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", &format!("/open/devices/{}/commands", id), StatusCode::OK, latency_ms).await;
...
async fn send_command(
    ...
    let command_params = payload.get("params")
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "POST", &format!("/open/devices/{}/command", id), StatusCode::OK, latency_ms).await;
...
async fn list_events(
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", &format!("/open/devices/{}/events", id), StatusCode::OK, latency_ms).await;
...
async fn list_all_events(
    ...
    let latency_ms = start.elapsed().as_millis() as i32;
    record_api_usage(&state, &tenant.id, Some(&key.id), "GET", "/open/events", StatusCode::OK, latency_ms).await;
