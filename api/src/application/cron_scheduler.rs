...
impl CronSchedulerService {
    ...
    async fn tick(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let now = Utc::now();
        let jobs = self.job_repo.find_due_jobs(None).await?;
        ...
        for job in due_jobs {
            ...
            let handle = tokio::spawn(async move {
                let _permit = permit;
                if let Err(e) = Self::execute_job(job, job_repo, run_repo, registry).await {
    ...
    async fn execute_job(
        ...
        let result: Result<ExecutionResult, ExecutorError> = if let Some(exec) = executor {
        ...
        let next_run = Self::compute_next_run_at(&job.cron_expression);
        // Note: next_run_at update is handled by update_run_stats in the repository impl
        // or would require a separate method. For now we log it.
