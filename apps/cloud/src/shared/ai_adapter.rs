...
pub struct AlarmAiPublisherAdapter {
    publisher: Arc<tinyiothub_agent::runtime::event::bus::AiEventPublisher>,
}

impl AlarmAiPublisherAdapter {
    pub fn new(publisher: Arc<tinyiothub_agent::runtime::event::bus::AiEventPublisher>) -> Self {
        Self { publisher }
    }
}
...
impl AlarmAiPublisherAdapter {
    pub fn publish_alarm_created(&self, event: crate::domains::alarm::AlarmEvent) {
        self.publisher
            .publish(tinyiothub_agent::runtime::event::types::AiEvent::AlarmCreated(event));
    }
}
...
impl crate::domains::tenant::hooks::WorkspaceEventPublisher for WorkspaceAiPublisherAdapter {
    fn publish_workspace_created(&self, workspace_id: String) {
        self.publisher
            .publish(tinyiothub_agent::runtime::event::types::AiEvent::WorkspaceCreated { workspace_id });
    }

    fn publish_workspace_deleted(&self, workspace_id: String) {
        self.publisher
            .publish(tinyiothub_agent::runtime::event::types::AiEvent::WorkspaceDeleted { workspace_id });
    }
}
