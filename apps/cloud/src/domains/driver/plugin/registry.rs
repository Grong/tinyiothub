...
impl PluginRegistry {
    ...
    fn create_config_plugin(
        ...
        match manifest.plugin_type {
            PluginType::Protocol => {
                let handler = crate::domains::driver::plugin::protocol::create_handler(config)?;
                Ok(handler)
            }
            PluginType::Notification => {
                let handler = crate::domains::driver::plugin::notification::create_handler(config, context)?;
                Ok(handler)
            }
            PluginType::Scheduler => {
                let handler = crate::domains::driver::plugin::scheduler::create_handler(config)?;
                Ok(handler)
            }
            PluginType::Storage => {
                let handler = crate::domains::driver::plugin::storage::create_handler(config, context)?;
                Ok(handler)
            }
            PluginType::Integration => {
                let handler = crate::domains::driver::plugin::integration::create_handler(config, context)?;
                Ok(handler)
            }
        }
    }
