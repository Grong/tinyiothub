...
use crate::domains::driver::plugin::{AppContext, PluginHandler, PluginManifest, PluginType};
...
/// 注册内置驱动到插件注册表
pub fn register_builtin_drivers(registry: &crate::domains::driver::plugin::PluginRegistry) {
    use tinyiothub_runtime::driver::{get_driver_list, get_supported_driver_names};
    ...
    for (info, name) in driver_infos.into_iter().zip(driver_names) {
        ...
        }) as crate::domains::driver::plugin::PluginFactory;
...
pub fn init_plugins(_context: Arc<AppContext>) -> Result<(), Error> {
    let registry = crate::domains::driver::plugin::get_global_registry();
