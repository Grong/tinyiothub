...
/// 创建驱动 API 路由
///
/// Stateless (driver metadata comes from the runtime registry), so the
/// router is generic over any composition state `S`.
...
async fn list_driver_names() -> Json<ApiResponse<Vec<String>>> {
    ...
    let driver_names = tinyiothub_runtime::driver::get_all_driver_names();
