...
use crate::domains::event::types::{EventStatistics, GroupBy, StatisticsGroup, StatisticsParams};
...
pub async fn get_event_overview(
    ...
    let real_time_filter = crate::domains::event::types::RealTimeFilter {
        device_ids: device_ids.clone(),
        event_types: None,
        source_types: None,
        acknowledged: Some(false), // Only unacknowledged
        min_level: Some(tinyiothub_core::models::event::EventLevel::Critical),
    ...
    // Build response
    let overview = build_overview_response(statistics, start_time, end_time, recent_critical_events);
