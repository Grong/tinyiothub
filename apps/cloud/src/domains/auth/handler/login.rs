...
async fn register(
    ...
    // 检查用户名是否已存在
    match state.auth_users().exists_by_username(username).await {
    ...
    // 检查手机号是否已注册
    match state.auth_users().exists_by_phone(phone).await {
    ...
    // 检查邮箱是否已注册（若提供了邮箱）
    if let Some(ref email) = email {
        match state.auth_users().exists_by_email(email).await {
    ...
    let user = match state.auth_users().create_user(&create_request).await {
        Ok(u) => u,
        Err(e) => {
            let msg = e.to_string();
...
/// 用户登录
async fn login(State(state): State<AppState>, Json(request): Json<LoginRequest>) -> Json<ApiResponse<LoginResponse>> {
        ...
        .await
    {
        Ok(Some(user)) => {
            ...
            if !state.harmonyos_enabled {
                // 更新最后登录时间
                if let Err(e) = state.auth_users().update_last_login(&user.id).await {
...
/// 用户登出
async fn logout(State(_state): State<AppState>, Json(_request): Json<LogoutRequest>) -> Json<ApiResponse<String>> {
