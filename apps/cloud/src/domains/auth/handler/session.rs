...
async fn get_profile(State(state): State<AppState>, claims: Claims) -> Json<ApiResponse<UserInfo>> {
    match state.auth_users().get_user_by_id(&claims.user_id).await {
        Ok(Some(user)) => {
...
/// 验证会话有效性
async fn validate_session(State(state): State<AppState>, claims: Claims) -> Json<ApiResponse<SessionInfo>> {
    // 检查用户是否仍然存在且未被禁用
    match state.auth_users().get_user_by_id(&claims.user_id).await {
        Ok(Some(user)) => {
