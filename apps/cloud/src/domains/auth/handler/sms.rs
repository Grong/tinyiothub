...
/// 检查发送频率限制
async fn check_rate_limit(state: &AppState, phone: &str, ip: Option<&str>) -> Result<RateLimitResult, StatusCode> {
...
/// 验证腾讯防水墙票据
async fn verify_captcha(state: &AppState, _ticket: &str, _randstr: &str, _ip: &str) -> Result<bool, StatusCode> {
