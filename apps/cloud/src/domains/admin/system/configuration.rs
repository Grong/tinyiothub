...
async fn get_network_config(State(state): State<AppState>, _claims: Claims) -> Json<ApiResponse<NetworkConfig>> {
    ...
    let config = NetworkConfig {
        ip_address: defaults.ip_address.clone(),
        subnet_mask: defaults.subnet_mask.clone(),
        gateway: defaults.gateway.clone(),
        dns_primary: defaults.dns_primary.clone(),
        dns_secondary: Some(defaults.dns_secondary.clone()),
        dhcp_enabled: false,
    };
...
async fn get_mqtt_config(State(state): State<AppState>, _claims: Claims) -> Json<ApiResponse<MqttConfig>> {
    ...
    let config = MqttConfig {
        broker_host: primary.host.clone(),
        broker_port: primary.port,
