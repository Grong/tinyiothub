...
/// All methods take `db_pool` from the cloud caller (Task 7): AgentPool
/// itself holds no storage handles.
impl AgentPool {
    // ========================================================================
    // Agent CRUD
    // ========================================================================

    pub async fn create_agent(&self, db_pool: &sqlx::SqlitePool, config: &AgentConfig) -> Result<String, AgentError> {
    ...
    pub async fn delete_agent(&self, db_pool: &sqlx::SqlitePool, agent_id: &str) -> Result<(), AgentError> {
    ...
    pub async fn get_agent(&self, db_pool: &sqlx::SqlitePool, agent_id: &str) -> Result<AgentInfo, AgentError> {
        let agent_id = agent_id.to_string();
    ...
    pub async fn get_agent_config(
        ...
        config_service::get_config_json(db_pool, agent_id).await
    }
    ...
    pub async fn set_agent_config(
        ...
        config_service::verify_agent_workspace(db_pool, agent_id, workspace_id).await?;
        config_service::set_config(db_pool, agent_id, config).await?;
        self.invalidate(agent_id);
    ...
    pub async fn tools_effective(
        ...
        let all_tools = {
            ...
            tool_service::load_all_tools(workspace_id, Some(db_pool.clone()), &runtime).await
        };
    ...
    pub async fn tools_toggle(
        ...
        config_service::verify_agent_workspace(db_pool, agent_id, workspace_id).await?;
        let mut config = config_service::get_config(db_pool, agent_id).await?;
        if enabled {
        ...
        config_service::set_config(db_pool, agent_id, &config_str).await?;
        self.invalidate(agent_id);
        Ok(())
    }
}
