...
/// SchedulerHandle；[`crate::domains::agent::loop_::thing_agent::scheduler::SchedulerHandle`]
/// 本身也实现它（单工作区直连，测试/桩用）。
#[async_trait::async_trait]
pub trait DirectiveSink: Send + Sync {
    /// 投递唤醒信号。语义与
    /// [`crate::domains::agent::loop_::thing_agent::scheduler::SchedulerHandle::enqueue`] 相同：
    /// 队列满拒绝用户指令（Rejected）、60s 同文去重（Duplicate）等
    /// 由实现方保证。
    fn enqueue(
        &self,
        signal: crate::domains::agent::loop_::thing_agent::types::WakeSignal,
    ) -> Result<(), crate::domains::agent::loop_::thing_agent::scheduler::EnqueueError>;

    /// O26 kill switch：清空该工作区调度器的待处理队列（ready queue + 未
    /// flush 的合并窗口）。语义与
    /// [`crate::domains::agent::loop_::thing_agent::scheduler::SchedulerHandle::drain`] 相同：不取消
...
mod tests {
    ...
    async fn scheduler_handle_is_a_directive_sink() {
        use crate::domains::agent::loop_::thing_agent::scheduler::Scheduler;
        ...
        assert_eq!(
            dup,
            Err(crate::domains::agent::loop_::thing_agent::scheduler::EnqueueError::Duplicate)
        );
    }
}
