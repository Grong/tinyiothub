...
use crate::test_utils::{create_test_token, response_parts, setup_test_app, setup_test_app_with_pool};
...
async fn test_token_refresh_invalid_token() {
    ...
    assert!(
        body["msg"].as_str().unwrap().contains("Invalid") || body["msg"].as_str().unwrap().contains("expired"),
...
async fn test_token_refresh_missing_token() {
    ...
    let response = app
        .oneshot(json_request("POST", "/api/v1/auth/token/refresh", json!({})))
        .await
        .unwrap();
...
async fn test_logout_without_token() {
    ...
    let response = app
        .oneshot(json_request("POST", "/api/v1/auth/token/logout", json!({})))
        .await
        .unwrap();
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Expected 200 for logout without token, got: {:?}",
        body
    );
...
async fn test_logout_blacklists_token() {
    ...
    let row: Option<(i64,)> = sqlx::query_as("SELECT 1 FROM token_blacklist WHERE token_hash = ? LIMIT 1")
        .bind(&token_hash)
        .fetch_optional(&pool)
        .await
        .unwrap();
