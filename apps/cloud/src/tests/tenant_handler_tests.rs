...
async fn test_list_tenants() {
    ...
    let response = app
        .oneshot(auth_request(
            "GET",
            "/api/v1/tenants/tenants?page=1&page_size=20",
            &token,
            None,
        ))
        .await
...
async fn test_list_plans() {
    let app = setup_test_app().await;
    let response = app
        .oneshot(public_request("GET", "/api/v1/tenants/plans", None))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
...
async fn test_list_api_keys() {
    ...
    let token = create_test_token("user-1", "tenant-1");
    let response = app
        .oneshot(auth_request("GET", "/api/v1/api-keys", &token, None))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let (_s, json) = response_parts(response).await;
...
async fn test_revoke_api_key_not_found() {
    ...
    let response = app
        .oneshot(auth_request(
            "DELETE",
            "/api/v1/api-keys/nonexistent-key-12345",
            &token,
            None,
        ))
        .await
...
async fn test_tenant_register_success() {
    ...
    let response = app
        .oneshot(public_request("POST", "/api/v1/tenants/register", Some(body)))
        .await
        .unwrap();
    let status = response.status();
...
async fn test_tenant_login_invalid_credentials() {
    ...
    let response = app
        .oneshot(public_request("POST", "/api/v1/tenants/login", Some(body)))
        .await
        .unwrap();
    let status = response.status();
...
async fn test_verify_token_invalid() {
    ...
    let response = app
        .oneshot(public_request(
            "GET",
            "/api/v1/tenants/verify?token=invalid-token-12345",
            None,
        ))
        .await
        .unwrap();
...
async fn test_get_tenant_usage() {
    ...
    let response = app
        .oneshot(auth_request(
            "GET",
            "/api/v1/tenants/tenants/tenant-1/usage",
            &token,
            None,
        ))
        .await
...
async fn test_get_tenant_usage_stats() {
    ...
    let response = app
        .oneshot(auth_request(
            "GET",
            "/api/v1/tenants/tenant-1/usage-stats",
            &token,
            None,
        ))
        .await
