...
| name / display_name | `name` 为命名模式替换后的机器名；`display_name` 取多语言模式替换后的 `zh` 值，缺 `zh` 取 `en`，再缺取 `name`。**properties/commands 的多语言 `display_name` 同样按此回退为单字符串**（thing_properties/thing_actions 表只存单字符串） |
| template_id | 所有实例化节点的 `template_id` 均填**场景包模板 id**（溯源 + 保留 `find_thing_template_events_by_thing` 等依赖 template_id 的关联路径） |
| default_knowledge | v1 写入 things 的 `linked_data` JSON，仅记录；Thing Agent 人设消费路径随 Thing Agent 配置演进接入 |
| events（事件定义元数据） | 双重语义澄清：模板层的 `events` 列存事件定义元数据（entity 模板原有机制）；场景包**节点**的 `events` 块实例化后写入该节点的 `linked_data.event_defs`；运行时 `events` 表是日志，不存定义 |
...
- `dry_run`：可选，默认 false。为 true 时只展开不落库，返回预览。**dry-run 是"只读"而非"无 IO"**：允许 SELECT 探测名称占用与配额（保证预览名称/数量与真实落库一致），但绝不写入。
...
- **落点（T2 裁定：扩展 import 闭环）**：导出结果为模板文件 JSON 下载；同时**扩展 `import_export` 流程**——`ParsedTemplate` 支持场景包字段（parameters/children/alarm_rules/resources/dashboard），导入时识别含 children 的 JSON 并按 `SceneTemplateFile` 解析注册为 workspace 模板。导出→编辑→注册闭环成立。
...
| 端点 | 变更 |
|---|---|
| `GET /api/marketplace/thing-templates` | 列表项增加 `is_composition`（bool，应用层解析 `device_info` 原文判定）与 `parameter_count`；支持 `?composition=true` 过滤。**过滤语义**：v1 模板量少，全量取出后应用层过滤再分页（DB 层不加过滤条件，分页计数基于过滤后结果）。**下沉触发条件**：模板总量 >50 或过滤成为实测瓶颈时，加 `kind` 列下沉 DB 过滤（届时做双写迁移） |
...
**结构摘要定义**：`{parameter_count, max_depth}`——`parameter_count` 为顶层 `parameters` 个数；`max_depth` 为**模板定义层的静态深度**（根为 1；`template_ref` 叶计 1 层，`scene_ref` 不深入被引用子树、计 1 层）。注意口径：`max_depth` 是静态结构指标，`node_count` 是运行时展开口径，两者不比较。示例：smart_campus（campus→building→floor→sensor）`max_depth = 4`。前端卡片显示如"2 参数 · 模板结构 4 层"（明示是模板静态结构，避免与实例化后真实深度混淆）。
...
| 展开后当前 thing 数 + node_count 超租户 `thing_limit` | 400，提示当前用量与上限（dry-run 同样校验） |
| 名称冲突（同 workspace 下重名） | 事务内 SELECT 探测下一个可用 `-N` 后缀（≤10），记 warning；撞唯一约束时重探测重试；超限返回 400「同名冲突过多，请手动指定名称」 |
| 反向导出：命名模式无法泛化 | 保留原名导出，记入 warnings |
| 落库中途失败 | 事务回滚，500，不留半棵树 |
| 设备节点 `required_fields`（如 address）留空 | 不阻断；结果页 warning 提示"设备节点需补充 address 后才能接入真实驱动" |
