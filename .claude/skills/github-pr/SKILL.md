...
Open or update a GitHub Pull Request for TinyIoTHub. Handles creating new PRs with a fully filled-out template body, and updating existing PRs (title, body sections, labels, comments). Use this skill whenever the user wants to open a PR, create a pull request, update a PR, edit PR description, add labels to a PR, or sync a PR after new commits — even if they don't say "PR" explicitly (e.g., "submit this for review", "push and open for merge").
...
- Which sections are marked with checklists vs free-form
- Any inline formatting conventions
...
Before drafting the PR body, actually run the commands the PR template's "Test plan" section asks for. Do not paraphrase results, do not write "tests pass" from memory, do not skip on the assumption that CI will catch it:
...
For changes that touch frontend files (`web/`):
```bash
cd web && pnpm type-check && pnpm build
```

For docs-only changes, skip the Rust/frontend batteries and run markdown lint + link-integrity checks per `AGENTS.md`.

Capture the tail of each command's output. You will paste the relevant excerpts (last 5–10 lines, any failures, any warnings) into the PR body's "Test plan" section as evidence that each checkbox was verified. If a command fails, stop and fix the underlying issue before drafting the PR — do not draft a PR on a broken tree.
...
When populating the "Test plan" section, check off each item you verified and paste the actual tail output — do not paraphrase. The reviewer will be looking for literal strings to diff against their own validation run.
...
- **Changes**: Bullet list of specific changes from the commits and diff.
- **Test plan**: Check off each item you actually ran, with evidence.
- **Related issues**: Link any issues (Fixes #123, Relates to #456).
- **Checklist**: Fill based on what the diff shows (architecture compliance, module structure, ApiResponseBuilder usage, Repository pattern, secrets check).

Draft a conventional commit-style PR title based on the changes (e.g., `feat(device): add temperature monitoring`, `fix(agent): handle empty config gracefully`, `chore(ci): add concurrency control`).
...
| **Link an issue** | Edit the "Related issues" section in the body |
...
2. Re-read the PR template. Analyze which sections are now stale based on the new changes — use the template's section names and field descriptions to identify what needs updating.

3. **If any of the new commits touch code (not pure docs)**, re-run the validation battery from Step 1a before updating the "Test plan" section. Stale validation evidence is worse than no evidence — it misleads the reviewer.
...
- **Never include personal/sensitive data** in PR content.
...
- **Conventional commits**: PR title and all commits must follow `type(scope): description` format per `AGENTS.md`.
- **Architecture compliance**: Verify the changes comply with the dependency direction, module structure, and anti-pattern rules in `AGENTS.md` before opening the PR.
