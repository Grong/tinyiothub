...
- **L3 self-heal approval enforcement:** Verify `execute_self_heal_action` checks `requireApproval` from policy before executing L3 actions. The `alarm-management.md` skill says L3 requires manual approval, but if the handler doesn't enforce this, an AI could autonomously execute `create_ticket` or `report_cloud` without human sign-off.
